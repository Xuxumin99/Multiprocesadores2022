...
int main()
{
    ...
    outputImage = fopen("toadout7.bmp","wb");    //Imagen transformada
    long ancho;
    long alto;
    ...
    //{
      //#pragma omp for schedule(dynamic)
        for (int i = 0; i < alto*ancho; i++) {
          ...
          if(linea<=2 || ( i==(inicioLin) && i<=(inicioLin+2) ) || ( i>=(finLin-2) && i==(finLin) ) || linea>=(alto-3) ){
            ...
            for(int j = -3; j <= 3 ; j++){
              a = i+(ancho*j); 
              valor = valor + ((ptr[a]+ptr[a-1]+ptr[a-2]+ptr[a-3]+ptr[a+1]+ptr[a+2]+ptr[a+3])/49);
            }   
