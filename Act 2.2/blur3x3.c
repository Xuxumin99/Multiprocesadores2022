...
int main()
{
    ...
    outputImage = fopen("toadout.bmp","wb");    //Imagen transformada
    long ancho;
    long alto;
    ...
    //{
      //#pragma omp for schedule(dynamic)
        for (int i = 0; i < alto*ancho; i++) {
          ...
          if(linea==0 || ( i==(inicioLin) /*&& i<=(inicioLin+2)*/ ) || ( /*i>=(finLin-2) &&*/ i==(finLin) ) || linea>=(alto-1) ){
            ...
            for(int j = -1; j <= 1 ; j++){
              a = i+(ancho*j); 
              valor = valor + ((ptr[a]+ptr[a-1]/*+ptr[a-2]+ptr[a-3]+ptr[a+1]+ptr[a+2]*/+ptr[a+1])/9);
            }   
